<?php
	...
	if(isset($_POST['continue']))
	{
	        $_SESSION['cardno']=$_POST['cardno'];
			 ...
			 $dcard=$_SESSION['cardno'];
			$selQry="select Cardno from account where `Cardno`='".$_POST['cardno']."'";
        $rsLogin=mysql_query($selQry);
        $count=mysql_num_rows($rsLogin);
		
		
						
		 if($count>0 && $cardno!=$dcard )
         {
              session_start();
		...
		else
	  {
		  //$str="Invalid Username Or Password";
           echo "<script> alert('Invalid Account or You entered your own card no')</script>";
	  }
   }
  ...
  <p>&nbsp;             </p>
   <table width="140" border="1" align="center">
    <tr>
      ...
      <input type="text" name="cardno" id="cardno" placeholder="Enter Cardno Number"/></td>
    </tr>
    <tr>
